...
public:
    ...
    Board(const grid_t &b, data_t v = 0) : grid_(b), data_(v) {}

    Board(const Board &board) = default;

    Board &operator=(const Board &b) = default;

    explicit operator grid_t &() { return grid_; }

    explicit operator const grid_t &() const { return grid_; }

    row_t &operator[](unsigned i) { return grid_[i]; }

    const row_t &operator[](unsigned i) const { return grid_[i]; }

    cell_t &operator()(unsigned i) { return grid_[i / 4][i % 4]; }

    const cell_t &operator()(unsigned i) const { return grid_[i / 4][i % 4]; }
    ...
    bool operator==(const Board &b) const { return grid_ == b.grid_; }
    ...
    void ReflectHorizontal() {
        for (int r = 0; r < 4; r++) {
            std::swap(grid_[r][0], grid_[r][3]);
            std::swap(grid_[r][1], grid_[r][2]);
        }
    }

    void ReflectVertical() {
        for (int c = 0; c < 4; c++) {
            std::swap(grid_[0][c], grid_[3][c]);
            std::swap(grid_[1][c], grid_[2][c]);
        }
    }
